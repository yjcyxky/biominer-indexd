...
# Step 1: 读取文件内容并计算哈希
def get_file_hash(path: Path, hash_type: str = "sha1") -> str:
...
def generate_deterministic_guid(file_path: Path) -> str:
...
def parse_meta_study(meta_path: Path) -> dict:
    """
    Parse the `dataset.txt` metadata file into a structured dictionary.

    Args:
        meta_path (Path): Path to the `dataset.txt` file.

    Returns:
        dict: Dictionary containing dataset metadata fields such as key, name, description, etc.
    """
    def strip_whitespace(s: str | list[str]) -> str | list[str]:
        if isinstance(s, list):
            return [item.strip() for item in s if len(item.strip()) > 0]
        else:
            return s.strip()
    ...
    with open(meta_path, "r") as f:
        for line in f:
            if ":" in line:
                key, val = strip_whitespace(line).split(":", 1)
                metadata[strip_whitespace(key)] = strip_whitespace(val)

    return {
        "key": strip_whitespace(metadata.get("key", "unknown")),
        "name": strip_whitespace(metadata.get("name", "Unnamed Study")),
        "description": strip_whitespace(metadata.get("description", "")),
        "citation": strip_whitespace(metadata.get("citation", "")),
        "pmid": strip_whitespace(metadata.get("pmid", "")),
        "groups": strip_whitespace(metadata.get("groups", "").split(";")),
        "tags": strip_whitespace(metadata.get("tags", "").split(";")),
        "total": strip_whitespace(metadata.get("total", 0)),
        "is_filebased": strip_whitespace(metadata.get("is_filebased", False)),
        "version": strip_whitespace(metadata.get("version", "v0.0.1")),
        "license": strip_whitespace(metadata.get("license", "")),
    }
...
def convert_study(
    study_dir: Path, output_dir: Path, version: str = "v0.0.1"
) -> Path:
    """
    Convert a cBioPortal-formatted dataset folder into a normalized dataset format.

    This includes:
    - Reading dataset.txt to create dataset.json
    - Combining metadata_table.tsv into metadata_table.parquet
    - Generating metadata_dictionary.json based on annotated headers
    ...
    """
    print(f"✅ Converting {study_dir} to {output_dir}...")
    study_dir = Path(study_dir)
    # Parse metadata
    meta_path = study_dir / "dataset.txt"
    if not meta_path.exists():
        raise FileNotFoundError("dataset.txt not found")

    try:
        dataset_meta = parse_meta_study(meta_path)
    except Exception as e:
        print(f"⚠️ Failed to parse the dataset metadata: {e}\n")
        return

    output_dir = Path(output_dir) / (version or dataset_meta.get("version", "v0.0.1"))
    ...
    if key != dirname:
        raise ValueError(
            f"The key in dataset.txt ({key}) does not match the directory name ({dirname})"
        )

    try:
        # Load and merge metadata_table.tsv
        metadata_files = [
            "metadata_table.tsv",
        ]
        dfs = []
    ...
    dataset_meta["version"] = version or dataset_meta.get("version", "v0.0.1")
    dataset_meta["is_filebased"] = False if dataset_meta.get("is_filebased") == "false" else True
...
@cli.command(help="Convert a cBioPortal-like dataset.")
@click.argument("study_dir", type=click.Path(exists=True, file_okay=False))
@click.argument("output_dir", type=click.Path())
@click.option("--version", type=str, default=None)
def convert(study_dir, output_dir, version):
    """
    CLI entry point to convert a cBioPortal-like dataset.

    STUDY_DIR is the path to a cBioPortal-like format study folder.
    OUTPUT_DIR is the output directory to save metadata_table.parquet, metadata_dictionary.json, dataset.json.
    """
    build_mappings()

    try:
        output_dir = convert_study(study_dir, output_dir, version)
    except Exception as e:
...
@cli.command(help="Batch convert cBioPortal-like datasets.")
@click.argument("input_dir", type=click.Path(exists=True, file_okay=False))
@click.argument("output_dir", type=click.Path())
@click.option("--version", type=str, default=None)
def batch_convert(input_dir, output_dir, version):
    """
    Batch convert cBioPortal-like datasets.
    """
    input_dir = Path(input_dir)
    output_dir = Path(output_dir)
    output_dir.mkdir(parents=True, exist_ok=True)

    for study_dir in input_dir.iterdir():
        if study_dir.is_dir() and study_dir.name != "datasets":
            try:
                output_dataset_dir = output_dir / study_dir.name
                convert_study(study_dir, output_dataset_dir, version)
            except Exception as e:
                print(f"⚠️ Failed to convert the dataset: {e}\n")
